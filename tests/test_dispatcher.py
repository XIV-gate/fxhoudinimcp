...
class TestDispatch:
    ...
    def test_nested_main_thread_dispatch_does_not_defer(self, monkeypatch):
        register_handler("test.nested", lambda **_: {"ok": True})
        monkeypatch.setattr(_disp, "HAS_HDEFEREVAL", True)
        _disp.hdefereval.executeInMainThreadWithResult.reset_mock()

        result = dispatch("test.nested", {})

        assert result["status"] == "success"
        _disp.hdefereval.executeInMainThreadWithResult.assert_not_called()
